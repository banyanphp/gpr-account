            ...
            <div class="col-md-12">
              ...
              <div class="tab-content">
                <div class="tab-pane fade active in" id="latest-post">
                  <blockquote>
                    <?php
                      $sel="SELECT btype,SUM(tcktamount) from `".TBL_BOOKING."` WHERE branch_idfk='". $_SESSION['branch'] ."' GROUP BY btype";
                  ...
                  <div class="height-sm" data-scrollbar="true">
                    <table class="table table-bordered">
                      ...
                      <tbody>
                        <?php 
                          global $database, $db;
                          $sel="SELECT * from `".TBL_BOOKING."` WHERE branch_idfk='". $_SESSION['branch'] ."'";
                          $result = $database->query($sel);
                          $j=1;
                          while($row = $database->fetch_array($result))
                ...
                <div class="tab-pane fade" id="purchase">
                  <blockquote>
                    <?php
                      $sel="SELECT btype,SUM(tcktamount) from `".TBL_BOOKING."` WHERE ctype = 'Advance' AND branch_idfk='". $_SESSION['branch'] ."'  GROUP BY btype";
                      $result = $database->query($sel);
                      while($row = $database->fetch_array($result)){
                      $formattedNum = number_format($row['SUM(tcktamount)'], 2);
                      echo "Total ". $row['btype']. " Collection= ". $formattedNum;
                  ...
                  <div class="height-sm" data-scrollbar="true">
                    <table class="table table-bordered">
                      ...
                      <tbody>
                        <?php
                          global $database, $db;
                          $sel="SELECT * from `".TBL_BOOKING."` WHERE ctype = 'Advance' AND branch_idfk='". $_SESSION['branch'] ."' ";
                          $result = $database->query($sel);
                          $j=1;
                          while($row = $database->fetch_array($result))
                ...
                <div class="tab-pane fade" id="email">
                  <blockquote>
                    <?php
                      $sel="SELECT btype,SUM(tcktamount) from `".TBL_BOOKING."` WHERE ctype = 'Credit' AND branch_idfk='". $_SESSION['branch'] ."' GROUP BY btype";
                      $result = $database->query($sel);
                      while($row = $database->fetch_array($result)){
                      $formattedNum = number_format($row['SUM(tcktamount)'], 2);
                      echo "Total ". $row['btype']. " Collection= ". $formattedNum;
                  ...
                  <div class="height-sm" data-scrollbar="true">
                    <table class="table table-bordered">
                      ...
                      <tbody>
                        <?php
                          global $database, $db;
                          $sel="SELECT * from `".TBL_BOOKING."` WHERE ctype = 'Credit' AND branch_idfk='". $_SESSION['branch'] ."' ";
                          $result = $database->query($sel);
                          $j=1;
                          while($row = $database->fetch_array($result))
